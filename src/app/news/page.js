...
async function getDataAll(pag) {
  try {
    const res = await fetch(`http://localhost:4000/api/postNews?limit=10&pag=${pag}`, { next: { revalidate: 1 } });
    const resCount = await fetch(`http://localhost:4000/api/postNews`, { next: { revalidate: 1 } });
